...
class CyberbullyingDetection():
    ...
    def detectBullying(self, text):
        ...
        command.execute("SELECT * FROM ml_model_slang")
        slangWords = pd.DataFrame(command, columns=['id', 'slang', 'word'])
        slangWords = slangWords.drop(['id'], axis=1)
        ...
        command.execute("SELECT * FROM ml_model_offensivewithseverity")
        offenseWords = pd.DataFrame(command, columns=['id', 'word', 'severity'])
        offenseWords = offenseWords.drop(['id'], axis = 1)
        offenseWords = offenseWords.replace(to_replace= '\\r', value= '', regex=True)
        
        command.execute("SELECT * FROM ml_model_negation")
        negationWords = pd.DataFrame(command, columns=['id', 'word'])
        negationWords = negationWords.drop(['id'], axis=1)
        ...
        model = pickle.load(open("cyberbullyingModel.sav", 'rb'))
        ...
        if (tweet['cyberbullying'].values == 'True'):
            command.execute("SELECT * FROM ml_model_ethnicityandraceglossary")
            ethnicityAndRaceGlossary = pd.DataFrame(command, columns=['id', 'word'])
            ethnicityAndRaceGlossary = ethnicityAndRaceGlossary.drop(['id'], axis = 1)
            ethnicityAndRaceGlossary = ethnicityAndRaceGlossary.replace(to_replace= '\\r', value= '', regex=True)
            ethnicityAndRaceGlossary.head()

            command.execute("SELECT * FROM ml_model_ageglossary")
            ageDataGlossary = pd.DataFrame(command, columns=['id', 'word'])
            ageDataGlossary = ageDataGlossary.drop(['id'], axis=1)
            ageDataGlossary = ageDataGlossary.replace(to_replace= '\\r', value= '', regex=True)
            ageDataGlossary.head()

            command.execute("SELECT * FROM ml_model_genderglossary")
            genderDataGlossary = pd.DataFrame(command, columns=['id', 'word'])
            genderDataGlossary = genderDataGlossary.drop(['id'], axis = 1)
            genderDataGlossary = genderDataGlossary.replace(to_replace= '\\r', value= '', regex=True)
            genderDataGlossary.head()

            command.execute("SELECT * FROM ml_model_releigionglossary")
            religiousDataGlossary = pd.DataFrame(command, columns=['id', 'word'])
            religiousDataGlossary = religiousDataGlossary.drop(['id'], axis=1)
            religiousDataGlossary = religiousDataGlossary.replace(to_replace= '\\r', value= '', regex=True)
            religiousDataGlossary.head()

            if tweet['cyberbullying'].all() == True:
                isEthnicityAndRace = []
                for row in tweet['tokens']:
                    temp = 0
                    for token in row:
            ...
            model = pickle.load(open("classifyModel.sav", 'rb'))
...
text = ["You're A STUPID BITCH kys because millions have died because of your retardedness i will never forgive and forget you, you old SONOFABITCH!!!"];
