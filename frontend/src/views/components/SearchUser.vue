                ...
                <div v-for="searchResult in search.results" :key="searchResult.username" class="p-3 hover:bg-theme-gray-light">
                    <RouterLink class="flex flex-row gap-3" :to="{ name: 'Profile', params: { username: searchResult.username } }">
                        ...
                        <div class="flex flex-col">
                            <div class="font-semibold">{{ searchResult.first_name+' '+searchResult.last_name }}</div>
                            <div class="text-gray-500 text-sm">{{ searchResult.username }}</div>
                        </div>
                    </RouterLink>
                </div>
            </div>
...
const { proceedSearchUser } = Search()
...
watch(() => search.value.query, (newVal, oldVal) => {
    ...
    if(newVal.length >= 2){
        ...
        proceedSearchUser(token, search.value.query).then((data) => {
