...
export default function Request_List() {
   ...
   const acceptRequest=(event)=>{
    axios.post(' http://127.0.0.1:8000/accounts/api/follow-user-action/1/'+ event.currentTarget.id + '/',{} ,{ headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
   ...
   const deleteRequest=(event)=>{
    axios.post('http://127.0.0.1:8000/accounts/api/follow-user-action/0/'+ event.currentTarget.id + '/',{} ,{ headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
           console.log(response.data);
