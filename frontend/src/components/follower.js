...
export default function Follower(){
    ...
    const fetchProfile = () =>{
        ...
        .then((response)=>{
            setValue(response.data.followers);
    ...
    const deletProfile = (event) => {
        axios.delete('http://127.0.0.1:8000/accounts/api/remove-follower/' + event.currentTarget.id + '/' ,{headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
        ...
        <div className='card'  style={{width:"100%", marginTop: '-7px', marginLeft:'-9px'}}>
            {value.map(data=>(
                ...
                <Card.Body>
                    ...
                    <Button style={{ width:"100px"}} className='d-inline p-2 ' id={data.pk} onClick={deletProfile} >Remove</Button>
