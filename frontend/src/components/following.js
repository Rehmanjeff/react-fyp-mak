...
export default function Following() {
    ...
    const fetchProfile = () =>{
        ...
        .then((response)=>{
            setValue(response.data.following);

        })
    }
    const deletProfile = (event) => {
        var btnid = event.currentTarget.id;
        axios.delete('http://127.0.0.1:8000/accounts/api/followers-list/' + event.currentTarget.id + '/' ,{headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
            let btn = document.getElementById(btnid);
            btn.innerHTML = 'Follow';
            btn.style.cssText = '--bs-btn-bg:#fff !important; --bs-btn-color:#50b7f5 !important; width:100px !important;';
            btn.onclick = function () { follow(btnid); };
    ...
    const follow = (id) => {
        console.log(id)
        axios.post('http://127.0.0.1:8000/accounts/api/follow-user/' + id  + '/',{},{headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
...
<div style={{ display: 'flex', height: '100vh', overflow: 'scroll initial' }}>
        ...
        <div className='card'  style={{width:"100%", marginTop: '-7px', marginLeft:'-9px'}}>
            {value.map(data=>(
                <Card key={data.pk}  className="text-center border-0" style={{width:"100%"}}>
                <Card.Body>
                    ...
                    <h5 className='d-inline p-2 ' >@{data.fields.username}</h5>
                    <div  className='d-inline p-2 '>
                    <Button style={{ width:"105px"}} id={data.pk}  onClick={deletProfile} >Following</Button>
                    </div>
                    </div>
                    <hr/>
                </Card.Body>
