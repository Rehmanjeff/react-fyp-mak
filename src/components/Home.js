...
const Home = () => {
    ...
    const fetchData = () =>{
        axios.get('http://127.0.0.1:8000/social/api/tweet/' ,{ headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response)=>{
    ...
    const postTweet = (event) => {
        ...
        axios.post('http://127.0.0.1:8000/social/api/get-tweet/tweet/', tweet,{ headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response) => {
            const element = document.getElementById('messageSuc');
            element.innerHTML += "<p><strong>Success:</strong>"+"- Tweet has been sent successfully posted</p><br>";
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'scroll initial' }}>
        ...
        <div className='card'  style={{width:"100%", marginTop: '-7px', marginLeft:'-9px'}}>
                <Card className="text-center border-0" style={{width:"100%"}}>
                    <Card.Body>
                    <Stack sx={{marginBottom:"10px",width: '100%', display: isActive ?"":"none"}} spacing={2}>
                        <Alert id='messageErr' severity="error">
                        </Alert>
                    </Stack>
                    <Stack sx={{marginBottom:"10px", width: '100%', display: isActiveSucc ?"":"none"}} spacing={2}>
                    ...
                    <Form  style={{display:"flex", justifyContent:"center"}}>
                        ...
                        className="me-2"
                        aria-label="Search"
                        style={{width: '350px',backgroundColor:" #d3d3d3 ", borderRadius: "1.375rem"}}
                        />
                    </Form>
                    </Card.Body>
                </Card>
                <Card className="text-center border-0" style={{width:"100%"}}>
                    <Card.Body>
                        ...
                        <h4 className='d-inline p-2 ' style={{color:"#808080"}} >Post a New Tweet</h4>
                        </div>
                    <Form >
                        <Form.Control
                        as="textarea" name='text' rows={3} placeholder='Enter your tweet' style={{border:'0px'}}
                        onChange={handleChange}
                        />
                        <div id='con' style={{display:'flex', justifyContent:"right"}}>
                        <Button  style={{width:"100px", marginTop:'10px'}}  variant="primary" onClick={postTweet}>Tweet</Button>
                        </div>
                    </Form>
