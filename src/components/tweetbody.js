...
export default function TweetBody(props) {
    ...
    const likePost = (event) => {
        ...
        axios.post(' http://127.0.0.1:8000/social/api/tweet-like/'+event.target.id+"/",{},{ headers: {"Authorization" : `Bearer ${token.access}`} })
        .then((response) => {
  ...
  return (
    <div>
        {props.data.map(tweet =>(
            tweet.map(subdata=>(
                <Card key={subdata.id} className="text-center border-0" style={{width:"100%"}}>
                    <Card.Body>
                        ...
                        <h5 className='d-inline p-2 ' >{subdata.user.first_name} {subdata.user.last_name}</h5>
                        <p style={{display:"flex", marginLeft:"122px", marginTop:"-39px"}}>@{subdata.user.username}</p>
                        </div>
                        <p  style={{textAlign:"left",marginLeft: "135px", fontSize:"14px"}}>{subdata.text}</p>
                        <div style={{marginTop:"30px"}} className="post_footer">
                        <ChatBubbleOutlineIcon style={{width:" 5em"}} fontSize="small"/>
                        <RepeatIcon style={{width:" 5em"}} fontSize="small" />
                        <FavoriteBorderIcon id={subdata.id} onClick={likePost} style={{width:" 5em"}} fontSize="small" />:
                        </div>
                        <br/>
                        <br/>

                        <p style={{float:"left", fontSize:"12px", margin:"0px"}}>Posted at: {subdata.updated_at}</p>
                        <br/>
                        <hr/>
                    </Card.Body>
                </Card>
