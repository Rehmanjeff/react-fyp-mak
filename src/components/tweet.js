...
export default function Tweet() {
  ...
  const updateTweet = (event) => {
    ...
    axios.patch('http://127.0.0.1:8000/social/api/get-tweet/tweet/'+ data.get("q") +'/', value,{ headers: {"Authorization" : `Bearer ${token.access}`} })
    .then((response) => {
        const element = document.getElementById('messageSuc');
        element.innerHTML += "<p><strong>Success:</strong>"+"- Tweet has been sent successfully update</p><br>";
        ...
        <Stack sx={{marginTop:"0px", width: '100%', display: isActiveSucc ?"":"none"}} spacing={2}>
            <Alert id='messageSuc' severity="success">
                    ...
                    <Card.Body>
                        ...
                        <h4 className='d-inline p-2 ' style={{color:"#808080"}} >Update your tweet</h4>
                        </div>
                        <p style={{textAlign:"left"}}>{data.get("y")}</p>
                    <Form >
                        <Form.Control
                        as="textarea" name='text' rows={6} placeholder='Enter your tweet'
                        onChange={handleChange}
                        />
                        <div id='con' style={{display:'flex', justifyContent:"right"}}>
                        <Button  style={{width:"140px", marginTop:'10px'}}  variant="primary"  onClick={updateTweet}>Update Tweet</Button>
                        </div>
                    </Form>
